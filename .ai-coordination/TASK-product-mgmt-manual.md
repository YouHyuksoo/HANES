# 작업 지시: HANES MES 제품수불관리(PRODUCT_MGMT) 메뉴 사용자 매뉴얼 생성

너는 HANES MES repo(C:\Project\HANES)에서 작업한다. 좌측 메뉴 "제품수불관리"(PRODUCT_MGMT 그룹)
4개 화면에 대해, 빠진 도움말을 소스 분석으로 작성해 채운 뒤, 4개 전체를 한 권의
단일 HTML 배포 매뉴얼로 묶는다.
...
- 참고: 제품 입고/출고는 FG_LABELS(시리얼)·PRODUCT_STOCKS 기준이다. 입고 시 BOX_NO 스탬프,
  PRODUCT_STOCKS 복합PK PRD_UID 센티넬('*') 등 제품재고 계약을 소스에서 확인해 반영한다.

## 2. 작성 대상 (코드 → route)  ※모두 신규 작성

### 2-A. user + operator 둘 다 작성 (4개)
PROD_RECEIVE            /product/receive
PROD_RECEIPT_CANCEL     /product/receipt-cancel
PROD_ISSUE              /product/issue
PROD_ISSUE_CANCEL       /product/issue-cancel

### 2-C. 이미 완비 — 손대지 말 것 (0개)
(없음)

## 3. manifest 등재
작성한 화면을 apps/frontend/public/help/manifest.json 의 categories 에 등재한다.
제품수불 전용 카테고리가 없으면 새 카테고리 { "key": "product", "title": "제품수불관리",
"items": [...] } 를 추가한다. title 은 각 화면 user/ko/{CODE}.md 의 frontmatter title 과
동일하게. 저장 시 BOM 금지, 2-space 들여쓰기 유지.

## 4. 매뉴얼(단일 HTML) 생성
도움말을 다 채운 뒤, 아래 러너로 4개를 한 권으로 만든다. 프론트 dev 서버가
http://localhost:3002 에 떠 있어야 한다(없으면 사용자에게 띄워달라고 요청).

  cd C:\Project\HANES
  $env:HELP_MANUAL_CODES = 'PROD_RECEIVE,PROD_RECEIPT_CANCEL,PROD_ISSUE,PROD_ISSUE_CANCEL'
  $env:MANUAL_TITLE   = 'HANES MES 제품수불관리 사용자 매뉴얼'
  $env:MANUAL_SYSTEM  = 'HARNESS MES'
  $env:MANUAL_VERSION = 'v1.0'
  $env:MANUAL_SLUG    = 'hanes-product-mgmt-manual'
  node C:\Users\hsyou\.claude\skills\help-manual-export\scripts\help-manual-export-runner.mjs

출력: docs/manuals/hanes-product-mgmt-manual-<날짜>.html (단일 파일, 이미지 base64 내장)
