# 작업 지시: HANES MES 출하관리(SHIPPING) 메뉴 사용자 매뉴얼 생성

너는 HANES MES repo(C:\Project\HANES)에서 작업한다. 좌측 메뉴 "출하관리"(SHIPPING 그룹)
9개 화면에 대해, 빠진 도움말을 소스 분석으로 작성해 채운 뒤, 9개 전체를 한 권의
단일 HTML 배포 매뉴얼로 묶는다.
...
SHIP_PACK_RESULT        /production/pack-result
SHIP_BOX_STOCK          /shipping/box-stock
SHIP_CONFIRM            /shipping/confirm
SHIP_HISTORY            /shipping/history
SHIP_RETURN             /shipping/return

### 2-B. user만 작성 (operator는 이미 존재) (3개)
SHIP_PACK               /shipping/pack
SHIP_ORDER              /shipping/order
SHIP_PALLET             /shipping/pallet

### 2-C. 이미 완비 — 손대지 말 것 (1개)
SHIP_PALLET_SHIP

## 3. manifest 등재
작성한 화면을 apps/frontend/public/help/manifest.json 의 categories 중 출하 관련
카테고리(key 예: "shipping")의 items 에 { "menuCode", "title", "path" } 로 추가한다.
title 은 각 화면 user/ko/{CODE}.md 의 frontmatter title 과 동일하게. 저장 시 BOM 금지,
2-space 들여쓰기 유지.

## 4. 매뉴얼(단일 HTML) 생성
도움말을 다 채운 뒤, 아래 러너로 9개를 한 권으로 만든다. 프론트 dev 서버가
http://localhost:3002 에 떠 있어야 한다(없으면 사용자에게 띄워달라고 요청).

  cd C:\Project\HANES
  $env:HELP_MANUAL_CODES = 'SHIP_PACK_RESULT,SHIP_PACK,SHIP_ORDER,SHIP_BOX_STOCK,SHIP_CONFIRM,SHIP_PALLET,SHIP_PALLET_SHIP,SHIP_HISTORY,SHIP_RETURN'
  $env:MANUAL_TITLE   = 'HANES MES 출하관리 사용자 매뉴얼'
  $env:MANUAL_SYSTEM  = 'HARNESS MES'
  $env:MANUAL_VERSION = 'v1.0'
  $env:MANUAL_SLUG    = 'hanes-shipping-manual'
  node C:\Users\hsyou\.claude\skills\help-manual-export\scripts\help-manual-export-runner.mjs

출력: docs/manuals/hanes-shipping-manual-<날짜>.html (단일 파일, 이미지 base64 내장)
