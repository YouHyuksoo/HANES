# 작업 지시: HANES MES 품질관리(QUALITY) 메뉴 사용자 매뉴얼 생성

너는 HANES MES repo(C:\Project\HANES)에서 작업한다. 좌측 메뉴 "품질관리"(QUALITY 그룹)
25개 화면에 대해, 빠진 도움말을 소스 분석으로 작성해 채운 뒤, 25개 전체를 한 권의
단일 HTML 배포 매뉴얼로 묶는다.
...
### 2-A. user + operator 둘 다 작성 (16개)
QC_IQC_HISTORY          /material/iqc-history
QC_CONCESSION           /material/concession
QC_SELF_INSPECT_HISTORY /quality/self-inspect-history
QC_SAMPLE_INSPECT       /production/sample-inspect
QC_OQC                  /quality/oqc
QC_OQC_HISTORY          /quality/oqc-history
QC_TRACE                /quality/trace
QC_CHANGE               /quality/change-control
QC_COMPLAINT            /quality/complaint
QC_CAPA                 /quality/capa
QC_FAI                  /quality/fai
QC_PPAP                 /quality/ppap
QC_SPC                  /quality/spc
QC_CONTROL_PLAN         /quality/control-plan
QC_AUDIT                /quality/audit
SYS_TRAINING            /system/training

### 2-B. user만 작성 (operator는 이미 존재) (2개)
QC_DEFECT               /quality/defect
QC_REWORK_INSPECT       /quality/rework-inspect

### 2-C. 이미 완비 — 손대지 말 것 (7개)
QC_IQC_ITEM, QC_IQC_PART_SPEC, QC_AQL, QC_IQC, QC_DEFECT_CODE, QC_INSPECT, QC_REQUEST_INSPECT

## 3. manifest 등재
작성한 화면을 apps/frontend/public/help/manifest.json 의 categories 중 품질 관련
카테고리(key 예: "quality")의 items 에 { "menuCode", "title", "path" } 로 추가한다.
title 은 각 화면 user/ko/{CODE}.md 의 frontmatter title 과 동일하게. 저장 시 BOM 금지,
2-space 들여쓰기 유지.

## 4. 매뉴얼(단일 HTML) 생성
도움말을 다 채운 뒤, 아래 러너로 25개를 한 권으로 만든다. 프론트 dev 서버가
http://localhost:3002 에 떠 있어야 한다(없으면 사용자에게 띄워달라고 요청).

  cd C:\Project\HANES
  $env:HELP_MANUAL_CODES = 'QC_IQC_ITEM,QC_IQC_PART_SPEC,QC_AQL,QC_IQC,QC_IQC_HISTORY,QC_CONCESSION,QC_DEFECT,QC_DEFECT_CODE,QC_REWORK_INSPECT,QC_INSPECT,QC_REQUEST_INSPECT,QC_SELF_INSPECT_HISTORY,QC_SAMPLE_INSPECT,QC_OQC,QC_OQC_HISTORY,QC_TRACE,QC_CHANGE,QC_COMPLAINT,QC_CAPA,QC_FAI,QC_PPAP,QC_SPC,QC_CONTROL_PLAN,QC_AUDIT,SYS_TRAINING'
  $env:MANUAL_TITLE   = 'HANES MES 품질관리 사용자 매뉴얼'
  $env:MANUAL_SYSTEM  = 'HARNESS MES'
  $env:MANUAL_VERSION = 'v1.0'
  $env:MANUAL_SLUG    = 'hanes-quality-manual'
  node C:\Users\hsyou\.claude\skills\help-manual-export\scripts\help-manual-export-runner.mjs

출력: docs/manuals/hanes-quality-manual-<날짜>.html (단일 파일, 이미지 base64 내장)
