...
- T-AI-PAGE-TOOL-WORKFLOW | 2026-06-22 | codex | HANES 전체 화면용 AI Page Tool Workflow 표준과 `/production/order` draft-only 파일럿 구현. 백엔드 manifest/read-only 후보조회, 프론트 `도구보기`/AI 패널 `채팅|도구|실행로그`, page hook, 작업지시 초안 반영 도구, chat `pageToolContext`, 작업지시 서버 자동채번 DTO 정합화. 저장은 사용자가 기존 버튼으로 실행 | evidence: JOURNAL 2026-06-22 19:37 Codex
- T-SHIP-ORDER-CANCEL | 2026-06-22 | claude | `/shipping/return`을 반품CRUD에서 **출하취소** 화면으로 재구성. 좌:통합 출하이력(박스+팔레트, 박스출하 팔레트번호 `*`)/우:팔레트·박스 상세/출하지시 단위 **단일 트랜잭션 취소**(팔레트분 reverse+CANCELED·박스분 cancel-ship-box, FG재고 1회 복원·상태전이)+SHIPPING_RETURNS 취소이력 자동기록(returnNo 시퀀스, 팔레트+박스 복원수량 항목화). BOX_MASTERS.SHIP_ORDER_NO/SHIPPED_AT 컬럼(JSHANES DDL)+3개 출하경로 stamp, 취소/역분개 in-tx 헬퍼 추출, 트랜잭션 내 pessimistic-lock 상태/ERP 재검증(동시성). i18n 4파일, 구조 테스트. SDD 8 Task+최종리뷰(Critical 0). 커밋 d3cf1f63..f77b36fa | evidence: 커밋 9068d021(취소)·f77b36fa(동시성/요약/테넌트), plan docs/superpowers/plans/2026-06-22-ship-order-cancel.md
