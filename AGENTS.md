...
### 2026-09-03 - 스킬 프로세스 남용으로 토큰만 소모 (중요)

- 기준정보검증 기능 착수 시 brainstorming → spec 작성 → spec 리뷰 서브에이전트 3회 → plan 작성 → plan 리뷰 서브에이전트 3회를 기계적으로 돌렸다.
- 결과: **코드 0라인, 문서 2개만 남기고 5시간 사용 한도 소진**. 리뷰 서브에이전트는 매번 repo를 새로 읽어 비용이 매우 크다.
- 사용자가 "이렇게 일하면 안 된다"고 강력히 지적했다.

교훈 (강제 규칙):

- 요구사항이 명확하고 구현이 기계적인 작업에 brainstorming/writing-plans/subagent-driven 같은 heavyweight 스킬 파이프라인을 전부 적용하지 않는다. 바로 구현한다.
- 스킬 문서의 절차(spec 리뷰 루프, plan 리뷰 루프, 태스크별 서브에이전트+2단계 리뷰)는 참고일 뿐이며, 리뷰 루프를 규격상 최대 횟수까지 돌리지 않는다. 문서 승인은 사용자에게 한 번만 묻는다.
- 서브에이전트 디스패치는 독립적이고 큰 작업 단위에만 사용한다. 작은 순차 작업을 나눠 디스패치하지 않는다.
- 토큰 비용이 커지는 방향으로 작업이 흐르고 있다고 판단되면 즉시 멈추고 사용자에게 방향을 확인한다.
