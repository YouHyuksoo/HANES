 ...
 *
 * BOM 행 식별 규칙: 상위품목코드(parentItemCode) + 하위품목코드(childItemCode) + 리비전(revision)이
 * 자연 복합키다. 수정/삭제는 이 세 값으로 대상을 지정한다. 백엔드 서비스는 이를
 * "parentItemCode::childItemCode::revision" 형태의 복합키 문자열(id)로 받는다.
...
export const BOM_MASTER_TOOL_MANIFEST: AiPageToolManifest = {
  ...
  tools: [
    ...
    },
    {
      ...
      label: 'BOM 행 수정',
      description:
        '기존 BOM 행을 수정한다. 대상은 parentItemCode·childItemCode·revision 세 값으로 지정한다(미입력 시 revision은 A로 간주). ' +
        '바꿀 항목만 넣는다: qtyPer(단위 소요량), seq(순서), processCode(공정코드), side(사이드 N/L/R), ' +
        'validFrom·validTo(유효기간 YYYY-MM-DD), remark(비고), useYn(사용여부 Y/N).',
      riskLevel: 'write',
      source: 'backend',
      inputSchema: {
        parentItemCode: { type: 'string', required: true },
    ...
    },
    {
      ...
      label: 'BOM 행 삭제',
      description:
        'parentItemCode·childItemCode·revision으로 지정한 BOM 행을 삭제한다(미입력 시 revision은 A로 간주).',
      riskLevel: 'write',
      source: 'backend',
      inputSchema: {
        parentItemCode: { type: 'string', required: true },
...
export class BomToolsProvider implements PageToolProvider {
  ...
  /** 복합키 id 생성: parentItemCode::childItemCode::revision */
  private bomKey(parentItemCode: string, childItemCode: string, revision: string): string {
    return `${parentItemCode}::${childItemCode}::${revision || 'A'}`;
  }
  ...
  private async updateBom(input: Record<string, unknown>, company?: string, plant?: string): Promise<AiPageToolWriteResult> {
    ...
    if (input.validFrom !== undefined) dto.validFrom = this.str(input.validFrom);
    ...
    await this.bomService.update(this.bomKey(parentItemCode, childItemCode, revision), dto, company, plant);
    return this.result(
      'updateBom',
      `BOM 행 ${parentItemCode} → ${childItemCode}(리비전 ${revision})를 수정했습니다.`,
      { parentItemCode, childItemCode, revision },
    );
  }
  ...
  private async deleteBom(input: Record<string, unknown>, company?: string, plant?: string): Promise<AiPageToolWriteResult> {
    ...
    await this.bomService.delete(this.bomKey(parentItemCode, childItemCode, revision), company, plant);
    return this.result(
      'deleteBom',
      `BOM 행 ${parentItemCode} → ${childItemCode}(리비전 ${revision})를 삭제했습니다.`,
      { parentItemCode, childItemCode, revision },
    );
  }
}
