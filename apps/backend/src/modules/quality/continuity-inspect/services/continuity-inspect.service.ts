...
export class ContinuityInspectService {
  ...
  /** 수리 시작과 같은 FG 행 잠금을 사용하여 수리실 인수 중 외부 판정/취소를 차단한다. */
  private async mutableLabelInTx(qr: QueryRunner, fgBarcode: string, company?: string, plant?: string): Promise<FgLabel> {
    // Oracle disallows FETCH FIRST + FOR UPDATE generated by findOne({ lock }).
    const parameters: string[] = [fgBarcode];
    let lockSql = 'SELECT FG_BARCODE FROM FG_LABELS WHERE FG_BARCODE = :1';
    if (company) { parameters.push(company); lockSql += ` AND COMPANY = :${parameters.length}`; }
    if (plant) { parameters.push(plant); lockSql += ` AND PLANT_CD = :${parameters.length}`; }
    await qr.query(`${lockSql} FOR UPDATE`, parameters);
    const label = await qr.manager.findOne(FgLabel, {
      where: { fgBarcode, ...(company ? { company } : {}), ...(plant ? { plant } : {}) },
    });
    if (!label) throw new NotFoundException(`FG 라벨을 찾을 수 없습니다: ${fgBarcode}`);
    this.assertTenantMatches('검사 라벨', { company, plant }, { label: 'fgLabel', company: label.company, plant: label.plant });
    const repair = await qr.manager.findOne(RepairOrder, {
      where: { fgBarcode, status: 'IN_REPAIR', company: label.company, plant: label.plant },
    });
    if (repair) throw new BadRequestException('수리 중인 FG 라벨은 수리관리 화면에서 재검사·종결해 주세요.');
    return label;
  }

  /** 구조검사 등의 공용 합격 플래그만으로 통전검사를 대체할 수 없다. */
  private async requireContinuityPassInTx(qr: QueryRunner, label: FgLabel): Promise<void> {
    const latest = await qr.manager.findOne(InspectResult, {
      where: { fgBarcode: label.fgBarcode, inspectType: 'CONTINUITY', company: label.company, plant: label.plant },
      order: { inspectAt: 'DESC', resultNo: 'DESC' },
    });
    if (label.inspectPassYn !== 'Y' || latest?.passYn !== 'Y') {
      throw new BadRequestException('통전검사 합격 후 육안검사를 진행하세요.');
    }
  }
  ...
  async updateFgLabelStatus(
    ...
    return this.tx.run(async (qr) => {
      const label = await this.mutableLabelInTx(qr, fgBarcode, company, plant);
      // 후공정/종료 상태 라벨은 외관검사 상태로 되돌릴 수 없다.
      const BLOCKED_SOURCE = ['PACKED', 'SHIPPED', 'VOIDED'];
      if (BLOCKED_SOURCE.includes(label.status)) {
        throw new BadRequestException(`현재 상태(${label.status})에서는 외관검사 상태로 변경할 수 없습니다.`);
      }
      await this.requireContinuityPassInTx(qr, label);
      label.status = status;
      return qr.manager.save(FgLabel, label);
    });
  }
  ...
  async visualInspect(
    ...
    const [result] = await this.visualInspectMany({ ...dto, fgBarcodes: [fgBarcode] }, company, plant);
    return result;
  }

  async findVisualPendingLabels(orderNo: string, company: string, plant: string): Promise<FgLabel[]> {
    return this.fgLabelRepo.createQueryBuilder('fg')
      .where('fg.orderNo = :orderNo', { orderNo })
      .andWhere('fg.company = :company', { company })
      .andWhere('fg.plant = :plant', { plant })
      .andWhere('fg.status = :status', { status: 'ISSUED' })
      .andWhere('fg.inspectPassYn = :passYn', { passYn: 'Y' })
      .andWhere(`EXISTS (
        SELECT 1 FROM INSPECT_RESULTS ir
        WHERE ir.FG_BARCODE = "fg"."FG_BARCODE"
          AND ir.COMPANY = "fg"."COMPANY" AND ir.PLANT_CD = "fg"."PLANT_CD"
          AND ir.INSPECT_TYPE = 'CONTINUITY' AND ir.PASS_YN = 'Y'
          AND NOT EXISTS (
            SELECT 1 FROM INSPECT_RESULTS newer
            WHERE newer.FG_BARCODE = ir.FG_BARCODE
              AND newer.COMPANY = ir.COMPANY AND newer.PLANT_CD = ir.PLANT_CD
              AND newer.INSPECT_TYPE = 'CONTINUITY'
              AND (newer.INSPECT_TIME > ir.INSPECT_TIME
                OR (newer.INSPECT_TIME = ir.INSPECT_TIME AND newer.RESULT_NO > ir.RESULT_NO))
          )
      )`)
      .orderBy('fg.issuedAt', 'ASC')
      .addOrderBy('fg.fgBarcode', 'ASC')
      .getMany();
  }

  async visualInspectBatch(
    dto: { orderNo: string; fgBarcodes: string[]; passYn: 'Y' | 'N'; errorCode?: string | null; errorDetail?: string | null; inspectData?: string | null; inspectorId?: string | null },
    company?: string,
    plant?: string,
  ): Promise<Array<{ inspectResult: InspectResult; fgLabel: FgLabel }>> {
    if (!dto.orderNo?.trim()) {
      throw new BadRequestException('일괄 육안검사 작업지시가 필요합니다.');
    }
    return this.visualInspectMany(dto, company, plant);
  }

  private async visualInspectMany(
    dto: { orderNo?: string; fgBarcodes: string[]; passYn: 'Y' | 'N'; errorCode?: string | null; errorDetail?: string | null; inspectData?: string | null; inspectorId?: string | null },
    company?: string,
    plant?: string,
  ): Promise<Array<{ inspectResult: InspectResult; fgLabel: FgLabel }>> {
    ...
    const barcodes = dto.fgBarcodes?.map((v) => v?.trim()).filter(Boolean) ?? [];
    if (barcodes.length === 0 || barcodes.length > 100) {
      throw new BadRequestException('육안검사 대상은 1~100건을 선택해야 합니다.');
    }
    if (new Set(barcodes).size !== barcodes.length) {
      throw new BadRequestException('중복된 FG 라벨이 있습니다.');
    }
    ...
    return this.tx.run(async (qr) => {
      ...
      for (const fgBarcode of [...barcodes].sort()) {
        ...
        if (['PACKED', 'SHIPPED', 'VOIDED'].includes(label.status)) {
          throw new BadRequestException(`현재 상태(${label.status})에서는 외관검사할 수 없습니다: ${fgBarcode}`);
        }
        if (dto.orderNo && label.status !== 'ISSUED') {
          throw new BadRequestException(`ISSUED 상태의 FG 라벨만 육안검사할 수 있습니다: ${fgBarcode} (${label.status})`);
        }
        if (dto.orderNo && label.orderNo !== dto.orderNo) {
          throw new BadRequestException(`선택한 작업지시와 FG 라벨의 작업지시가 일치하지 않습니다: ${fgBarcode}`);
        }
        await this.requireContinuityPassInTx(qr, label);
        locked.set(fgBarcode, label);
      }

      const results: Array<{ inspectResult: InspectResult; fgLabel: FgLabel }> = [];
      for (const fgBarcode of barcodes) {
        const label = locked.get(fgBarcode)!;
        const inspectResultNo = await this.seqGenerator.getNo('INSPECT_RESULT', qr);
        const inspectResult = qr.manager.create(InspectResult, {
          resultNo: inspectResultNo, prodResultNo: null, inspectType: 'VISUAL', inspectScope: 'FULL',
          passYn: dto.passYn,
          ...
          fgBarcode, inspectorId: dto.inspectorId ?? null, inspectAt: new Date(),
          company: company ?? label.company, plant: plant ?? label.plant,
        });
        ...
        results.push({ inspectResult: savedInspect, fgLabel: savedLabel });
      }
      return results;
    });
  }
  ...
  async inspect(
    ...
    const result = await this.tx.run(async (queryRunner) => {
      ...
      let inspectedLabel: FgLabel | null = null;
      if (dto.fgBarcode) {
      ...
      if (dto.passYn === 'Y') {
        ...
        const issuedLabel = inspectedLabel!;
  ...
  async reInspect(
    ...
    const result = await this.tx.run(async (queryRunner) => {
      ...
      if (label.inspectPassYn !== 'N') {
        throw new BadRequestException('불합격(inspectPassYn=N) 바코드만 재검사할 수 있습니다.');
      }
  ...
  async voidLabel(
    ...
    return this.tx.run(async (qr) => {
      const label = await this.mutableLabelInTx(qr, fgBarcode, company, plant);
      if (label.status === 'VOIDED') throw new BadRequestException('이미 취소된 라벨입니다.');
      if (['PACKED', 'SHIPPED'].includes(label.status)) {
        throw new BadRequestException(`후공정이 진행된 FG 라벨(${fgBarcode})은 취소할 수 없습니다. 출하/팔레트/박스부터 먼저 정리해 주세요.`);
      }
      label.status = 'VOIDED';
