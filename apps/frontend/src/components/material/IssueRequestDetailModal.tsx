              ...
              <table className="w-full min-w-[860px] text-sm">
                <thead className="sticky top-0 bg-surface text-xs text-text-muted">
                  <tr className="border-b border-border">
                    ...
                    <th className="px-3 py-2 text-right">{t('material.issue.issuableQty', { defaultValue: '가용(IQC합격)' })}</th>
                    <th className="px-3 py-2 text-right">{t('material.issue.pendingIqcQty', { defaultValue: '미검사' })}</th>
                ...
                <tbody>
                  {items.length === 0 ? (
                    <tr>
                      <td colSpan={12} className="px-3 py-10 text-center text-text-muted">
                        {t('common.noData')}
                      </td>
                    </tr>
                  ) : items.map((item) => (
                    <tr key={`${item.requestId ?? request.requestNo}-${item.seq ?? item.itemCode}`} className="border-b border-border/70 last:border-0">
                      ...
                      {/* 가용(IQC합격)이 잔여보다 적으면 승인/출고 시 부족 — 강조 */}
                      <td className={`px-3 py-2 text-right tabular-nums ${toNumber(item.issuableQty) < remainQty(item) ? 'font-semibold text-red-600 dark:text-red-400' : ''}`}>
                        {formatNumber(item.issuableQty)}
                      </td>
                      <td className={`px-3 py-2 text-right tabular-nums ${toNumber(item.pendingIqcQty) > 0 ? 'text-amber-600 dark:text-amber-300' : 'text-text-muted'}`}>
                        {toNumber(item.pendingIqcQty) > 0 ? formatNumber(item.pendingIqcQty) : '-'}
                      </td>
