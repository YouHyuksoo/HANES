...
test('IQC modal uses expanded compact scanner-first layout', () => {
  ...
  assert.match(modalSource, /className="flex h-\[calc\(75vh-32px\)\] max-h-\[620px\] flex-col gap-1\.5 overflow-hidden"/);
  assert.doesNotMatch(modalSource, /min-h-\[620px\]/);
  assert.match(modalSource, /className="grid grid-cols-12 gap-1\.5 rounded-lg bg-background p-1\.5"/);
  assert.match(modalSource, /className="col-span-5 relative"/);
...
test('IQC modal keeps judge body visible without separate bottom form blocks', () => {
  assert.match(modalSource, /className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-border"/);
  assert.match(modalSource, /className="grid min-h-0 flex-1 grid-cols-12"/);
