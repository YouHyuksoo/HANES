...
export default function RequestTable({ data }: RequestTableProps) {
  const columns = useMemo<ColumnDef<IssueRequest>[]>(() => [
    { accessorKey: 'requestNo', header: '요청번호', size: 160 },
    { accessorKey: 'requestDate', header: '요청일', size: 100 },
    {
      accessorKey: 'workOrderNo', header: '작업지시', size: 160,
      cell: ({ getValue }) => (
        <span className="text-primary font-medium">{getValue() as string}</span>
      ),
    },
    {
      id: 'itemCount', header: '품목수', size: 70,
      cell: ({ row }) => <span>{row.original.items.length}건</span>,
    },
    {
      accessorKey: 'totalQty', header: '총수량', size: 100,
      cell: ({ getValue }) => (
