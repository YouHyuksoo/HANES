...
// 원칙: 처리성 화면(병합 대상 선택)은 대상이 되는 LOT(재고>0·NORMAL·예약0·입고완료)만 서버 조건으로 받고,
...
const service = fs.readFileSync('apps/backend/src/modules/material/services/lot-merge.service.ts', 'utf8');

test('/material/lot-merge uses server paging instead of fetching everything', () => {
...
test('backend restricts merge candidates in the DB query, not in memory', () => {
  assert.match(service, /\.where\('stock\.qty > 0'\)/);
