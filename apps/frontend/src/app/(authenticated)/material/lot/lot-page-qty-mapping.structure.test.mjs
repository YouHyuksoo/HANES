...
// 화면 모델(MatLotItem)은 lotColumns.tsx 로 추출되었다 — 인터페이스 단정은 그 파일을 본다.
const columnsSource = readFileSync(new URL('./lotColumns.tsx', import.meta.url), 'utf8');
...
test('lot list maps API currentQty into qty so grid does not crash', () => {
  ...
  assert.match(columnsSource, /currentQty\?:\s*number/);
