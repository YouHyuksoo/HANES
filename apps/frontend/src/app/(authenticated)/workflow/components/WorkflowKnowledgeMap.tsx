...
import { expandKnowledgeNeighborhood, workflowKnowledgeCatalog, KNOWLEDGE_CATEGORIES, type KnowledgeCategory, type KnowledgeNode, type KnowledgeRelation } from "@harness/shared";
...
export function WorkflowKnowledgeMap() {
  ...
  const graph = useMemo(() => {
    ...
    const semantic = snapshot.relations.filter((v): v is KnowledgeCategory => (KNOWLEDGE_CATEGORIES as readonly string[]).includes(v));
    const expanded = expandKnowledgeNeighborhood([center.id, ...expandedNodeIds], semantic);
    const relations: KnowledgeRelation[] = [...expanded.relations];
    if (snapshot.relations.includes("evidence")) for (const relation of workflowKnowledgeCatalog.relations) if (relation.category === "evidence" && expanded.nodes.some((node) => node.id === relation.source)) relations.push(relation);
    const ids = new Set([center.id, ...relations.flatMap((relation) => [relation.source, relation.target])]);
    const nodes = workflowKnowledgeCatalog.nodes.filter((node) => ids.has(node.id));
    return { center, nodes, relations };
  }, [center, expandedNodeIds, snapshot.relations]);
  const model = useMemo(() => { if (!graph || !center) return { nodes:[], edges:[] }; const positioned = safeLayout(KNOWLEDGE_LAYOUTS[snapshot.layout], graph); const result = createKnowledgeViewModel(graph, positioned, { centerId:center.id, selectedNodeId, viewMode:snapshot.view, relationCategories:snapshot.relations }); return { ...result, nodes:result.nodes.map((node) => ({ ...node, data:{ ...node.data, center:node.id===center.id, evidenceStatus:graph.relations.find((r)=>r.source===node.id||r.target===node.id)?.evidenceStatus ?? "undocumented" } })) }; }, [graph, center, selectedNodeId, snapshot.layout, snapshot.view, snapshot.relations]);
  const selected = workflowKnowledgeCatalog.nodes.find((node) => node.id === selectedNodeId) ?? center;
  const selectNode = (nodeId: string) => { setSelectedNodeId(nodeId); setExpandedNodeIds((ids) => ids.includes(nodeId) ? ids : [...ids, nodeId]); };
  const recenter = (nodeId: string) => { update((m)=>m.setCenter(nodeId)); setSelectedNodeId(nodeId); setExpandedNodeIds([]); setFitSignal((v)=>v+1); };
  ...
  return <div className="flex h-[calc(100dvh-8rem)] min-h-0 flex-col overflow-hidden rounded-lg border border-border bg-background text-text">
    <header className="bg-card"><div className="flex items-end justify-between gap-4 border-b border-border px-4 py-3"><div><p className="font-mono text-[10px] uppercase tracking-[0.24em] text-text-muted">HANES · KNOWLEDGE BLUEPRINT</p><h1 className="text-lg font-semibold">업무 지식 관계도</h1></div><p className="hidden max-w-lg text-right text-xs text-text-muted lg:block">노드를 선택하면 한 단계 관계가 펼쳐집니다. 중심 이동은 상세 패널에서 명시적으로 실행합니다.</p></div><KnowledgeSearch onSelect={selectNode} invalidCenter={snapshot.invalidCenter} /><KnowledgeToolbar layout={snapshot.layout} view={snapshot.view} canGoBack={snapshot.canGoBack} canGoForward={snapshot.canGoForward} onLayout={setLayout} onView={setView} onBack={()=>update((m)=>{m.goBack();})} onForward={()=>update((m)=>{m.goForward();})} onReset={reset} onFitView={()=>setFitSignal((v)=>v+1)} onCopyLink={()=>void copyLink()} copyStatus={copyStatus} /></header>
