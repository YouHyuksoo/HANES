...
const columnsSource = readFileSync(new URL('./inspectionHistoryColumns.tsx', import.meta.url), 'utf8');
...
test('inspection history exposes inspection type as filter and grid column', () => {
  ...
  // 컬럼 정의는 inspectionHistoryColumns.tsx로 분리됨
  assert.match(columnsSource, /accessorKey:\s*'inspectType'/);
  assert.match(columnsSource, /외관검사/);
  assert.match(columnsSource, /단자검사/);
  assert.match(columnsSource, /통전검사/);
});
...
test('inspection history uses shared DateRangeFilter defaulting to today', () => {
  ...
  assert.match(pageSource, /const\s+\[fromDate,\s*setDateFrom\]\s*=\s*useState\(\(\)\s*=>\s*getTodayLocal\(\)\)/);
  assert.match(pageSource, /const\s+\[toDate,\s*setDateTo\]\s*=\s*useState\(\(\)\s*=>\s*getTodayLocal\(\)\)/);
  // 로컬 중복 함수 제거됨
