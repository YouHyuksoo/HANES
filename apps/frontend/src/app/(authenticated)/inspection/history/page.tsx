 ...
 * 4. 헤더 "차트로 보기" 토글 — 조회된 동일 데이터를 InspectionHistoryCharts로 집계 표시 (재조회 없음)
 */

import { useState, useMemo, useEffect, useCallback } from "react";
import { useTranslation } from "react-i18next";
import {
  Zap, RefreshCw, Search, BarChart3, Table2,
} from "lucide-react";
...
import InspectionHistoryCharts from "./InspectionHistoryCharts";
    ...
    <div className="h-full flex flex-col overflow-hidden p-6 gap-3 animate-fade-in">
      <div className="flex justify-between items-center flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant={isChartView ? "primary" : "secondary"}
            size="sm"
            onClick={() => setViewMode(isChartView ? "grid" : "chart")}
            aria-pressed={isChartView}
          >
            {isChartView
              ? <><Table2 className="w-4 h-4 mr-1" />{t("inspection.history.viewGrid", "목록으로 보기")}</>
              : <><BarChart3 className="w-4 h-4 mr-1" />{t("inspection.history.viewChart", "차트로 보기")}</>}
          </Button>
        ...
        <CardContent className="h-full p-4">
          {isChartView ? (
            <div className="h-full flex flex-col gap-4 min-h-0">
              <div className="flex-shrink-0">{filterBar}</div>
              <div className="flex-1 min-h-0">
                <InspectionHistoryCharts data={data} fromDate={fromDate} toDate={toDate} />
              </div>
            </div>
          ) : (
            <DataGrid
