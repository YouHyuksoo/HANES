...
/**
 * @file src/app/(authenticated)/quality/spc/components/SpcChartUploadModal.tsx
 * @description SPC 관리대상(SPC_CHARTS) 엑셀 업로드 모달 — BomUploadModal과 동일한 미리보기→업로드 2단계 UX.
 */
...
export default function SpcChartUploadModal({ isOpen, onClose, onComplete }: Props) {
  ...
  const handleDownloadTemplate = async () => {
    const res = await api.get("/quality/spc/charts/upload/template", { responseType: "blob" });
    const url = URL.createObjectURL(res.data);
    const a = document.createElement("a");
    a.href = url;
    a.download = "SPC_CHARTS_template.xlsx";
    a.click();
  ...
  const handlePreview = async (f: File) => {
    ...
    try {
      ...
      const res = await api.post("/quality/spc/charts/upload/preview", formData, {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await api.post("/quality/spc/charts/upload", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      if (res.data.success) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={t("quality.spc.hv.chartUploadTitle", "관리항목 엑셀 업로드")}
      size="xl"
      footer={
        <>
    ...
    >
      <div className="mb-4">
        ...
        />
        <p className="text-xs text-text-muted mt-1">{t("quality.spc.hv.xlsxOnlyHint", ".xlsx 파일만 업로드 가능합니다. 품목코드/공정코드는 기준정보에 등록돼 있어야 합니다.")}</p>
      </div>
              ...
              <table className="w-full text-xs">
                <thead className="sticky top-0 bg-surface dark:bg-gray-800 border-b border-border">
                  <tr className="text-text-muted">
                    ...
                    <th className="px-2 py-2 text-left">공정코드</th>
                    <th className="px-2 py-2 text-left">특성명</th>
                    <th className="px-2 py-2 text-center w-16">서브그룹</th>
                    <th className="px-2 py-2 text-center w-20">규격(L/T/U)</th>
                  ...
                  {preview.rows.map((r) => {
                    ...
                    return (
                      <tr key={r.row} className="border-t border-border/50">
                        ...
                        <td className="px-2 py-1.5 font-mono truncate max-w-[100px]" title={r.itemCode}>{r.itemCode}</td>
                        <td className="px-2 py-1.5 font-mono truncate max-w-[100px]" title={r.processCode}>{r.processCode}</td>
                        <td className="px-2 py-1.5 truncate max-w-[140px]" title={r.characteristicName}>{r.characteristicName}</td>
                        <td className="px-2 py-1.5 text-center">{r.subgroupSize ?? "-"}</td>
                        <td className="px-2 py-1.5 text-center text-text-muted">{[r.lsl, r.target, r.usl].map((v) => v ?? "-").join("/")}</td>
      ...
      {stage === "done" && result && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            ...
            <StatCard label="건너뜀" value={result.skipped} color="amber" />
...
export function ErrorTable({ errors }: { errors: { row: number; message: string }[] }) {
  return (
