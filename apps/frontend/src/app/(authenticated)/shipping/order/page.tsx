 ...
 * 2. **상태 흐름**: DRAFT -> CONFIRMED -> CLOSED (정본 어휘 COM_CODES.SHIP_ORDER_STATUS, 미완료 판정은 @harness/shared isShipOrderOpen)
 * 3. API: GET/POST/PUT/DELETE /shipping/orders
 */
import { useState, useEffect, useCallback, useMemo } from "react";
import { isShipOrderOpen } from "@harness/shared";
...
export default function ShipOrderPage() {
  ...
  // 기간(출하예정일) 밖이지만 미완료(DRAFT/CONFIRMED/SHIPPING)라 includeOpen으로 포함된 행 — 종결 상태는 CLOSED 다
  const outOfRangeNos = useMemo(() => {
    ...
    for (const o of data) {
      ...
      if (!inRange && isShipOrderOpen(o.status)) set.add(o.shipOrderNo);
    }
    return set;
