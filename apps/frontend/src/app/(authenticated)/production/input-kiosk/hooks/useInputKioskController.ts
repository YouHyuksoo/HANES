...
/**
 * @file hooks/useInputKioskController.ts
 * @description 실적입력(가공) 키오스크의 상태·게이트·모달 제어 로직 단일 출처
 *
 * 초보자 가이드:
 * - 기존 input-kiosk/page.tsx 에 있던 상태 로직을 그대로 옮긴 것이다. 화면 배치가 다른 두 페이지
 *   (input-kiosk = 3열 배치, input-kiosk-b = 작업 순서 스테퍼 배치)가 같은 훅을 쓴다.
 *   게이트 규칙(점검·자재·초물/중물/종물·정지)이 두 화면에서 갈라지지 않게 하려는 목적이다.
 * - 이 훅은 API 호출과 zustand kioskStore 갱신을 담당하고, 그리기는 각 page.tsx 가 담당한다.
 * - 모달 열림 상태와 핸들러도 여기서 관리한다. 모달 렌더는 components/InputKioskModals.tsx 가 맡는다.
 *
 * 자주검사 트리거:
...
import { useEquipWorkers } from './useEquipWorkers';
import { useEquipStop } from './useEquipStop';
...
import type { SgLabelPrintHandle } from '../components/SgLabelPrintHost';
...
export function useInputKioskController() {
  ...
  const productionType: 'TRIAL' | 'MASS' = firstInspectDone ? 'MASS' : 'TRIAL';
  ...
  /** 실적입력 버튼 활성 조건 — 두 화면이 같은 식을 쓴다 */
  const canSubmit = allInterlockDone && !hasPendingDelegate && !isMidBlock && !equipStop.isStopped;

  return {
    // 스토어 스냅샷
    selectedEquip, selectedJobOrder, selectedWorkers, workerNames, interlock, savedResultCount,
    hasPendingDelegate, midInspectDone,
    // 목록/상태
    equips, equipStop, historyKey,
    firstInspectDone, lastInspectDone, isMidBlock, isLastBlock, productionType,
    dailyInspectAt, workerInspectAt, dailyInspectResult, workerInspectResult,
    midNotifyPct, midBlockPct, progressPct,
    allInterlockDone, canSubmit, submitDisabledReasons, materialScanDisabledReasons, consumableScanDisabledReasons,
    carrierFlags, carrierRequired, outputCarrier,
    guide, sgPrinterRef,
    // 모달 열림 상태
    isJobOrderOpen, setIsJobOrderOpen,
    isEquipSelectOpen, setIsEquipSelectOpen,
    isWorkerOpen, setIsWorkerOpen,
    isDailyInspectOpen, setIsDailyInspectOpen,
    isWorkerInspectOpen, setIsWorkerInspectOpen,
    isMaterialScanOpen, setIsMaterialScanOpen,
    isConsumableScanOpen, setIsConsumableScanOpen,
    isDefectOpen, setIsDefectOpen,
    isEquipStopOpen, setIsEquipStopOpen,
    isManagerCallOpen, setIsManagerCallOpen,
    selfInspectTiming, setSelfInspectTiming,
    // 핸들러
    restoreEquipmentCurrentState, handleRemoveWorker, handleWorkerConfirm, handleJobOrderConfirm,
    refreshDailyInspect, refreshWorkerInspect,
    handleSaved, handleResultSaved, handleSelfInspectDone, handleOpenDefect,
  };
}

export type InputKioskController = ReturnType<typeof useInputKioskController>;
