      ...
      <div className="flex-shrink-0 border-b border-border bg-card">
        ...
        <div className="flex h-14 items-center gap-3 border-b border-border/50 bg-surface/50 px-4">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div className="flex h-11 min-w-[15rem] flex-1 items-center gap-2 overflow-hidden rounded-lg border border-border bg-card px-3">
              ...
              {selectedJobOrder ? (
                ...
                <button data-testid="kiosk-joborder-open" onClick={() => selectedEquip && onOpenJobOrder()} disabled={!selectedEquip}
                  ...
                  className={`shrink-0 whitespace-nowrap rounded bg-primary px-2.5 py-1 text-xs font-semibold text-white transition-colors hover:bg-primary/90 disabled:cursor-not-allowed disabled:bg-surface disabled:text-black/60 dark:text-white/60 ${selectedEquip ? 'animate-pulse' : ''}`}>
            ...
            <div className={`flex h-11 min-w-0 shrink items-center gap-1.5 overflow-hidden rounded-lg border border-border bg-card pl-2 pr-1.5 xl:pr-3 ${
              ...
              {selectedWorkers.map(w => (
                <span key={w.id} className="inline-flex items-center gap-1 rounded-full border border-green-600 px-2 py-0.5 text-xs font-bold text-green-700 dark:border-green-400 dark:text-green-400">
                  ...
                  <span className="max-w-[64px] truncate xl:max-w-none" title={w.workerName}>{w.workerName}</span>
              ...
              {selectedWorkers.length === 0 && selectedEquip && (
                <span className="flex items-center gap-1 whitespace-nowrap text-xs font-bold text-red-600 dark:text-red-400" title={t('kiosk.header.workerRequired')}>
                  ...
                  <span className="hidden xl:inline">{t('kiosk.header.workerRequired')}</span>
                </span>
              )}
            </div>
          </div>
          ...
          {/* 설비정지 / 관리자호출 — 헤더 우측 고정.
              평소에는 전체화면 버튼과 같은 아이콘 크기로 두고(작업지시/작업자 칸을 밀어내지 않도록),
              진행 중일 때만 경과시간까지 펼쳐 보여준다. */}
          <div className="flex shrink-0 items-center gap-2">
            <button
              type="button"
              ...
              className={`inline-flex h-11 shrink-0 items-center justify-center gap-1.5 rounded-lg border-2 transition-colors disabled:cursor-not-allowed disabled:border-border disabled:text-black/40 dark:disabled:text-white/40 ${
                isStopped
                  ...
                  : 'w-11 border-border text-black/70 hover:border-red-500 hover:text-red-600 dark:text-white/70 dark:hover:border-red-400 dark:hover:text-red-400'
              }`}
            >
            ...
            <button
              ...
              className={`inline-flex h-11 shrink-0 items-center justify-center gap-1.5 rounded-lg border-2 transition-colors disabled:cursor-not-allowed disabled:border-border disabled:text-black/40 dark:disabled:text-white/40 ${
                isCalling
                  ...
                  : 'w-11 border-border text-black/70 hover:border-amber-500 hover:text-amber-600 dark:text-white/70 dark:hover:border-amber-400 dark:hover:text-amber-400'
              }`}
            >
