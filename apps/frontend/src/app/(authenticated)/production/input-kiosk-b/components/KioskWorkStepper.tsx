 ...
 * @description B 배치 우측 열 — 실제 작업 순서(설비 점검 → 자재 로트 → 공정샘플검사 → 실적입력) 스테퍼
 *
 * 초보자 가이드:
...
function StepBadge({ n, state }: { n: number; state: StepState }) {
  ...
  return (
    <span data-testid={`kiosk-b-step-${n}`} data-state={state} className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${cls}`}>
      {n}
    </span>
  );
}
...
export default function KioskWorkStepper({ c }: { c: InputKioskController }) {
  ...
  const pendingSteps = dones.slice(0, 3).map((d, i) => (d ? null : i + 1)).filter((n): n is number => n !== null);
  ...
  return (
    <div data-testid="kiosk-b-stepper" className="flex h-full min-h-0 flex-col overflow-hidden rounded-md border-2 border-slate-300 bg-card dark:border-slate-600">
      ...
      <section className="flex shrink-0 flex-col gap-1 border-t-2 border-slate-300 px-4 pt-3 pb-3 dark:border-slate-600">
          <StepHeader
            n={4}
            state={stateOf(3)}
