...
/**
 * @file production/subprocess-kitting-b/components/KitReferencePanels.tsx
 * @description B 배치 좌측 하단 참조 영역 — 설비 자재 장착(왼쪽) | 이전 공정 SFG 스캔(오른쪽) 좌우 분할
 *
 * 초보자 가이드:
...
import InputSgScanPanel from '../../subprocess-kitting/components/InputSgScanPanel';
import type { SubprocessKittingController } from '../../subprocess-kitting/hooks/useSubprocessKittingController';

export default function KitReferencePanels({ c }: { c: SubprocessKittingController }) {
  ...
  return (
    <div data-testid="subkit-b-reference-panels" className="grid h-[300px] shrink-0 grid-cols-2 gap-3">
      {/* 왼쪽: 설비 자재 장착 */}
      ...
      <section className={box}>
        <div className={head}>
          ...
          {t('production.subprocess.inputSgScan', '이전 공정 SFG 스캔')}
