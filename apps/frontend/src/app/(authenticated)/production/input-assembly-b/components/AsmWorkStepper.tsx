...
/**
 * @file production/input-assembly-b/components/AsmWorkStepper.tsx
 * @description B 배치 우측 열 — 작업 순서 스테퍼 5단계
 *   ① 작업자 ② 설비 점검 ③ 설비 자재 장착 ④ 반제품 SFG 스캔 ⑤ FG 발행·확정
 *
 * 초보자 가이드:
 * - 위에서 아래로 읽는 순서가 곧 작업 순서다. 단계 번호 배지는 완료(초록)/현재 막힘(빨강)/대기(회색)
 *   세 상태만 가진다. "왜 FG 발행이 안 눌리는지"를 이 열 한 곳에서 보게 하는 것이 목적이다.
 * - 단계 완료 판정은 컨트롤러(useInputAssemblyController)의 interlock·canIssue 값을 그대로 쓴다.
 *   여기서 새 규칙을 만들지 않는다 — A안과 판정이 갈리면 두 시안을 비교할 수 없다.
 * - ②③단계의 실제 스캔 입력은 왼쪽 참조 영역(KitReferencePanels)에 있다. 이 열은 그 진행 상태만 보여준다.
 * - ④단계는 세로형 AsmResultEntry 를 쓰고 열 하단에 고정한다(화면이 낮아도 버튼이 보인다).
...
import AsmResultEntry from './AsmResultEntry';
import AsmMaterialMountModal from './AsmMaterialMountModal';
...
function StepBadge({ n, state }: { n: number; state: StepState }) {
  ...
  return (
    <span data-testid={`asm-b-step-${n}`} data-state={state} className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${cls}`}>
      {n}
    </span>
  );
}
...
export default function AsmWorkStepper({ c }: { c: InputAssemblyController }) {
  ...
  const {
    ...
    sgList, issuedFg, sgReady, issueDisabledReason,
  ...
  const doneSg = sgList.length > 0 && sgReady;
  const doneIssue = !!issuedFg;

  // ③ 자재 장착은 완료 신호가 없다 — EquipMaterialMountPanel 이 장착 완료를 컨트롤러에 알려주지 않고,
  //    canIssue 게이트에도 들어가지 않는다. 그래서 "완료"를 주장하지 않고 진행 체인에서도 뺀다.
  //    (가공은 interlock.materialScanDone 이 있어 완료 표시가 가능하다. 조립에 같은 인터록을 만들려면 별도 작업.)
  ...
  return (
    <div data-testid="asm-b-stepper" className="flex h-full min-h-0 flex-col overflow-hidden rounded-md border-2 border-slate-300 bg-card dark:border-slate-600">
      {/* 헤더 — 이번 FG 한 건의 진행. 참조 상자·작업지도서 상자와 같은 어두운 제목 띠 */}
      <div className="flex h-12 shrink-0 items-center justify-between bg-slate-800 px-4 text-white dark:bg-slate-700">
        ...
        {selectedOrder ? (
          <span className="flex items-baseline gap-1.5">
            ...
            <span className="text-xs text-slate-300">{t('production.inputAssembly.scannedSg', '투입 SFG')}</span>
          </span>
        ) : (
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
        ...
        <section className="flex flex-col gap-2 border-b border-border px-4 py-3">
          ...
          <div className="flex gap-2 pl-10">
            <button
              type="button"
              data-testid="asm-material-scan-open"
            ...
            >
              ...
              {t('production.inputAssembly.materialScan', '자재 스캔')}
            </button>
          </div>
        </section>
        ...
        <section className="flex flex-col gap-2 border-b border-border px-4 py-3">
          <StepHeader
            n={4}
            state={stateOfGate(2)}
            title={t('production.inputAssembly.sgScan', '반제품 SFG 스캔')}
            right={stateText(sgList.length > 0 && sgReady, `SFG ${sgList.length}`)}
          />
          <p className="pl-10 text-xs text-text-muted">
            {t('production.inputAssembly.stepSgHint', '오른쪽 아래 스캔 패널에서 BOM 이 요구하는 반제품 SFG 라벨을 모두 담으세요.')}
          </p>
        </section>
      </div>

      {/* ⑤ FG 발행 · 확정 — 스크롤 영역 밖(열 하단 고정) */}
      <section className="flex shrink-0 flex-col gap-2 border-t-2 border-slate-300 px-4 pt-3 pb-3 dark:border-slate-600">
        <StepHeader
          n={5}
          state={stateOfGate(3)}
          title={t('production.inputAssembly.issueAndConfirm', 'FG 발행 · 확정')}
          right={<span className="text-text-muted">{issuedFg ? t('production.inputAssembly.awaitConfirm', '실물 스캔 대기') : t('production.inputAssembly.awaitIssue', '발행 전')}</span>}
        />
        <div className="pl-10">
          <AsmResultEntry
            canIssue={canIssue}
          ...
          />
          {!canIssue && !issuedFg && pendingSteps.length > 0 && (
            <p data-testid="asm-b-pending-steps" className="pt-2 text-center text-xs font-semibold text-red-600 dark:text-red-400">
