...
/**
 * @file src/app/(authenticated)/production/input-assembly-b/page.tsx
 * @description 실적입력(조립) B 배치 시안 — 작업 순서 스테퍼형 (2026-09-21 시안 B 채택 검토용)
 *
 * 초보자 가이드:
 * 화면 구성:
 *   ① 상단 컨텍스트 띠(어두운 슬레이트): 설비 · 작업지시 · 대차 · 준비 안내 · 초기화
 *   ② 좌측: 작업지도서(가장 넓게) / 아래 참조 영역 좌우 분할(설비 자재 장착 | 반제품 SFG 스캔)
 *   ③ 우측: 작업 순서 스테퍼(작업자 → 설비 점검 → 자재 장착 → 반제품 SFG → FG 발행·확정)
 *   ④ 하단: 지표 띠(CT·UPH·UPPH·계획·투입 SFG, 한 양식)
 *
 * 상태·게이트·모달 로직은 기존 화면과 같은 useInputAssemblyController 를 쓴다. 이 파일은 배치만 다르다.
 * 기존 /production/input-assembly 는 그대로 두었다. 채택되면 그쪽 배치를 이 구성으로 바꾸고 이 라우트는 정리한다.
 */
...
import AssemblyPrepGuideModal from '../input-assembly/components/AssemblyPrepGuideModal';
...
import { toJobOrderPick, useInputAssemblyController } from '../input-assembly/hooks/useInputAssemblyController';
import AsmContextBar from './components/AsmContextBar';
import AsmReferencePanels from './components/AsmReferencePanels';
import AsmWorkStepper from './components/AsmWorkStepper';
import AsmMetricsStrip from './components/AsmMetricsStrip';

export default function InputAssemblyBPage() {
  const { t } = useTranslation();
  const c = useInputAssemblyController();
  const {
    selectedOrder, selectedWorkers, workerNames, processCode, sgList, productivityRevision,
    ...
    equipCode, equipName, refreshInspectStatus, selectOrder, handleWorkerSelect, fgPrinterRef, guide,
    equipStop, isEquipStopOpen, setIsEquipStopOpen, isManagerCallOpen, setIsManagerCallOpen,
  } = c;

  return (
    <div data-testid="asm-b-page" className="flex h-full flex-col overflow-hidden bg-background">
      {/* ① 컨텍스트 띠 */}
      <AsmContextBar c={c} />
      ...
      {/* FG(완제품) 라벨 자동 출력 호스트 — A안과 동일, 오프스크린 렌더 후 Print Agent 전송 */}
