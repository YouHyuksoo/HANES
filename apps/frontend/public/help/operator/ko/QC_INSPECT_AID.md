...
summary: INSPECT_AIDS 테이블 전체 컬럼·DB 매핑, CRUD·사진 업로드(multer uploads/inspect-aids)·만료임박(expiring) API, 유효기간 계산 로직, 공통코드 INSPECT_AID_TYPE/INSPECT_AID_STATUS와 트러블슈팅. 한도견본은 LIMIT_SAMPLES로 분리됨
tags: [품질, 기준정보, 검사홀더, 운영]
keywords: [INSPECT_AIDS, INSPECT_AID_TYPE, INSPECT_AID_STATUS, HOLDER, VALID_TO, expiring, expiryState, daysToExpiry, IMAGE_URL, uploads/inspect-aids, multer, inspect-aids, 검사홀더, 지그, 멀티테넌시, 트러블슈팅, LIMIT_SAMPLES]
related: [QC_LIMIT_SAMPLE, MST_PART, MST_PROCESS, EQUIP_INSPECT_ITEM_MASTER]
---

# 검사보조구 관리 — 운영 가이드

## 시스템 목적·역할
검사홀더·지그를 `INSPECT_AIDS` 테이블로 관리합니다. 기존에는 PPAP 제출 체크박스 외에 개념이 없었고, 고객 감사(24P·25P) 대응으로 2026-09-09에 신설했습니다. 사진은 설비점검항목 마스터와 같은 multer 디스크 업로드 패턴(`uploads/inspect-aids/`)을 쓰고, 유효기간 만료·임박은 서버가 `VALID_TO` 기준으로 계산해 내려줍니다.

> **2026-09-16 분리**: 양품/불량 한도견본(`LIMIT_OK`/`LIMIT_NG`) 76건은 `LIMIT_SAMPLES`로 이관됐고, 한도견본 전용 컬럼 `DEFECT_CODE`·`INSPECT_TYPE`·`REQUIRED_YN`·`SORT_ORDER` 4개는 DROP됐습니다. 공통코드 `INSPECT_AID_TYPE`의 `LIMIT_OK`/`LIMIT_NG`는 `USE_YN='N'`으로 내려가 있습니다. 통전·단자검사 대조는 이제 `LIMIT_SAMPLES`를 읽습니다 — 이 테이블을 조인하지 마세요. 마이그레이션: `2026-09-16_limit_samples_migrate.sql`

## 데이터 구조
```
INSPECT_AIDS (PK COMPANY + PLANT_CD + AID_CODE)
  ├─ AID_TYPE     → COM_CODES 'INSPECT_AID_TYPE' (HOLDER)
...
| 유형 | `AID_TYPE` | NOT NULL. 공통코드 `INSPECT_AID_TYPE`. DTO `@IsIn(HOLDER)`. 컬럼과 인덱스는 유지 — 향후 다른 보조구 유형 확장 여지. |
...
|------|------|------|------|
| 목록·필터·페이징 | `GET /master/inspect-aids?page&limit&aidType&status&itemCode&processCode&useYn&search` | — | 읽기. 정렬 AID_CODE. 각 행에 `expiryState`/`daysToExpiry`(30일 기준) 포함. |
...
## 사전 설정 (마스터·공통코드)
- 공통코드 `INSPECT_AID_TYPE`(HOLDER만 활성), `INSPECT_AID_STATUS`(ACTIVE/EXPIRED/RETIRED) — 마이그레이션이 MERGE.
- 마스터: 품목(`ITEM_MASTERS`), 공정(`PROCESS_MASTERS`).
...
- 초기 데이터: 이관 후 화면이 비지 않도록 홀더 시드 3건(`HD-CONT-001`, `HD-CRIMP-001`, `HD-VISUAL-001`)이 들어 있습니다.

## 운영 절차
1. 마이그레이션 `2026-09-09_inspect_aids.sql` → `2026-09-16_limit_samples_migrate.sql` 순서로 적용.
2. 홀더·지그 실물에 코드 라벨을 붙이고, 같은 코드로 등록 + 사진 업로드.
...
| 한도견본이 목록에 안 보임 | 2026-09-16에 `LIMIT_SAMPLES`로 이관됨 | [양불마스터](/master/limit-sample) 화면에서 조회 |
...
| 유형 라벨이 코드로 보임 | i18n `comCode.INSPECT_AID_TYPE.*` 누락 | 4개 locale에 키 추가 |
| 메뉴가 안 보임 | `MENU_CATEGORY_ITEMS`/`ROLE_MENU_PERMISSIONS` 미등록 | 마이그레이션 4) 블록 확인 |

## 데이터·연계
- 테이블: `INSPECT_AIDS`; 파일: `uploads/inspect-aids/`
- 참조: `COM_CODES`(`INSPECT_AID_TYPE`, `INSPECT_AID_STATUS`), `ITEM_MASTERS`, `PROCESS_MASTERS`(모두 논리 참조)
...
- 마이그레이션: `apps/backend/src/migrations/2026-09-09_inspect_aids.sql`, `2026-09-16_limit_samples_migrate.sql`
