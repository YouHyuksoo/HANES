...
> Superseded: 이 문서는 LOT 범위별 수동 AQL rule 관리 1차 설계다. 2026-06-20에 사용자가 `품목별 AQL + 업체별 검사강도 + ISO 2859-1 자동계산 + 업체 검사모드 자동전환` 정책을 확정했으므로, 최신 설계는 `docs/specs/2026-06-20-iqc-aql-z14-policy-design.md`를 기준으로 한다.
