...
const reportRoot = path.resolve(`docs/reports/hanes-quality-menu-scenario-qa-${reportDate}`);
...
const resultPath = path.join(reportRoot, 'quality-menu-result.json');
...
const staticMenuMap = {
  QC_IQC: {
    ...
    route: '/material/iqc',
    title: '수입검사(IQC)',
    api: '/material/iqc-history/pending-arrivals',
    tables: ['IQC_LOGS', 'MAT_LOTS'],
    processPattern: [/등록/, /검사/, /처리/],
  },
  QC_IQC_HISTORY: {
    slug: 'quality-iqc-history',
    route: '/material/iqc-history',
    title: 'IQC 이력',
    api: '/material/iqc-history?limit=20',
    tables: ['IQC_LOGS'],
    processPattern: [/조회/, /검색/],
  },
  QC_SAMPLE_INSPECT: {
    slug: 'quality-sample-inspect',
    route: '/production/sample-inspect',
    title: '시료검사',
    api: '/production/sample-inspect-input?limit=20',
    tables: ['SAMPLE_INSPECT_RESULTS'],
    processPattern: [/등록/, /검사/],
  },
  QC_TRACE: {
    slug: 'quality-trace',
    route: '/quality/trace',
    title: '추적관리',
    api: '/quality/trace?limit=20',
    tables: ['TRACE_LOGS'],
    processPattern: [/조회/, /검색/, /추적/],
  },
  QC_DEFECT: {
    slug: 'quality-defect',
    route: '/quality/defect',
    title: '불량관리',
    api: '/quality/defect-logs?limit=20',
    tables: ['DEFECT_LOGS'],
    processPattern: [/등록/, /추가/, /불량/],
  },
  QC_REWORK_INSPECT: {
    slug: 'quality-rework-inspect',
    route: '/quality/rework-inspect',
    title: '재작업검사',
    api: '/quality/reworks?limit=20&status=INSPECT_PENDING',
    tables: ['REWORK_ORDERS', 'REWORK_INSPECTS'],
    processPattern: [/검사/, /처리/, /판정/],
  },
  QC_REQUEST_INSPECT: {
    slug: 'quality-request-inspect',
    route: '/quality/request-inspect',
    title: '검사의뢰',
    api: '/production/self-inspect/delegates?limit=20',
    tables: ['SELF_INSPECT_RESULTS'],
    processPattern: [/처리/, /판정/, /검사/],
  },
  QC_SELF_INSPECT_HISTORY: {
    slug: 'quality-self-inspect-history',
    route: '/quality/self-inspect-history',
    title: '자주검사이력',
    api: '/production/self-inspect/history?limit=20',
    tables: ['SELF_INSPECT_RESULTS'],
    processPattern: [/조회/, /검색/],
  },
  MAT_SHELF_LIFE: {
    slug: 'material-shelf-life',
    route: '/material/shelf-life',
    title: '유통기한',
    api: '/material/shelf-life?limit=20',
    tables: ['MAT_LOTS'],
    processPattern: [/조회/, /검색/],
  },
  MAT_SHELF_LIFE_HISTORY: {
    slug: 'material-shelf-life-history',
    route: '/material/shelf-life-history',
    title: '유통기한이력',
    api: '/material/shelf-life/reinspect?limit=20',
    tables: ['IQC_LOGS'],
    processPattern: [/조회/, /검색/],
  },
  MAT_SHELF_LIFE_REINSPECT: {
    slug: 'material-shelf-life-reinspect',
    route: '/material/shelf-life-reinspect',
    title: '유통기한재검사',
    api: '/material/shelf-life/reinspect?limit=20',
    tables: ['IQC_LOGS', 'MAT_LOTS'],
    processPattern: [/등록/, /재검사/, /검사/],
  },
};
...
async function discoverQualityMenus() {
  const tree = await apiGet('/menu-categories/tree');
  const categories = Array.isArray(tree) ? tree : tree.data ?? [];
  const category = categories.find((item) => item.categoryCode === 'QUALITY' && item.isActive !== 'N');
  if (!category) throw new Error('QUALITY menu category not found in /menu-categories/tree');
...
async function exerciseSearch(page) {
  ...
  const clicked = await clickFirst(root, [/검색/, /조회/, /추적/]).catch(() => null);
...
async function resetSearchAndRefresh(page) {
  ...
  const refresh = await clickFirst(root, [/새로고침/, /조회/, /추적/]).catch(() => null);
  await page.waitForLoadState('networkidle', { timeout: 6000 }).catch(() => {});
  await page.waitForTimeout(900);
  ...
  await addStep('action-availability', '신규/수정/삭제 가능 여부', `${pageInfo.title} 화면의 저장성 프로세스 진입과 행 액션 노출 여부를 확인합니다.`, [
    ...
    return {
      ...
      notes: [
        ...
        `저장성 후보 버튼: ${buttons.filter((name) => /등록|저장|추가|삭제|수정|취소|입고|출고|불량|검사|판정|처리|재검사/.test(name)).join(', ') || '없음'}`,
      ],
    };
  });

  await addStep('save-validation-policy', '저장 검증 및 중복 방어 정책', `${pageInfo.title} 화면의 저장성 처리 경계와 중복 방어 검증 방식을 기록합니다.`, [
    '품질 데이터를 변경하는 저장/삭제 버튼은 이 공통 메뉴 스윕에서 임의 실행하지 않는다.',
    ...
    return {
      ...
      notes: [
        ...
        `중복 방어 공통 시나리오: 동일 LOT/동일 검사키 재처리 방어는 페이지별 저장형 상세 시나리오에서 수행`,
        `저장성 후보 버튼: ${buttons.filter((name) => /등록|저장|추가|삭제|수정|취소|입고|출고|불량|검사|판정|처리|재검사/.test(name)).join(', ') || '없음'}`,
      ],
    };
  });
...
function renderPageReport(pageResult) {
  const featureRows = [
    ['좌측 메뉴 등록', `/menu-categories/tree QUALITY/${pageResult.menuCode}`, '실행', `등록 순서 ${pageResult.sortOrder}; 실제 등록 메뉴 기준`],
    ...
    ['저장 검증', '직접 저장 미실행', '별도 시나리오 대상', '품질 데이터 변경이 필요한 저장형 업무는 페이지별 안전 데이터 시나리오에서 수행'],
    ['중복 방어', '동일 LOT/동일 검사키 재처리 방어', '별도 시나리오 대상', '공통 스윕에서는 방어 정책과 버튼/API 경계를 기록'],
...
<head>
  <meta charset="utf-8">
  <title>품질관리 등록 메뉴 상세 시나리오 QA 목차</title>
  <style>
    body { margin: 0; font-family: Arial, "Malgun Gothic", sans-serif; background: #f5f7fb; color: #111827; }
    header { background: #172033; color: #fff; padding: 28px 34px; }
    main { padding: 24px 34px 48px; }
...
<body>
  <header>
    <h1>품질관리 등록 메뉴 상세 시나리오 QA 목차</h1>
  ...
  <main>
    <section class="card">
      <p>이 보고서는 <code>/api/v1/menu-categories/tree</code>에서 <code>QUALITY</code> 카테고리에 등록된 하위 메뉴만 대상으로 생성했습니다. 정적 메뉴에는 있으나 실제 좌측 메뉴에 등록되지 않은 화면은 제외했습니다.</p>
    </section>
    <section class="card">
      <table>
...
async function writeAggregate(category, pages, results) {
  ...
  await fs.writeFile(resultPath, JSON.stringify({
    ...
    menuSource: {
      ...
      excludedStaticQualityCodes: Object.keys(staticMenuMap).filter((code) => !pages.some((page) => page.menuCode === code)),
    },
    pages: results,
  }, null, 2), 'utf8');
...
async function main() {
  ...
  const { category, pages } = await discoverQualityMenus();
  if (aggregateMode) {
    const results = [];
    for (const pageInfo of pages) {
      const pageResultPath = path.join(pageDir, `${pageInfo.slug}.json`);
      if (existsSync(pageResultPath)) results.push(JSON.parse(await fs.readFile(pageResultPath, 'utf8')));
    }
    if (results.length === 0) throw new Error('no page results to aggregate');
    const aggregate = await writeAggregate(category, pages, results);
    console.log(JSON.stringify({ status: aggregate.status, source: '/menu-categories/tree QUALITY', pages: results.length, passed: results.filter(r => r.status === 'PASS').length, failed: aggregate.failed.length, indexPath, resultPath }, null, 2));
  ...
  console.log(JSON.stringify({
    ...
    source: '/menu-categories/tree QUALITY',
