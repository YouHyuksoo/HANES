...
const reportRoot = path.resolve(`docs/reports/hanes-consumables-menu-scenario-qa-${reportDate}`);
...
const resultPath = path.join(reportRoot, 'consumables-menu-result.json');
...
const staticMenuMap = {
  CONS_MASTER: {
    ...
    route: '/consumables/master',
    title: '소모품관리 마스터',
    api: '/consumables?limit=20',
    tables: ['CONSUMABLE_MASTERS'],
    processPattern: [/등록/],
  },
  CONS_LABEL: {
    slug: 'consumables-label',
    route: '/consumables/label',
    title: '라벨발행',
    api: '/consumables/label/masters',
    tables: ['CONSUMABLE_MASTERS'],
    processPattern: [/발행/, /인쇄/],
  },
  CONS_RECEIVING: {
    slug: 'consumables-receiving',
    route: '/consumables/receiving',
    title: '입고관리',
    api: '/consumables/logs?limit=20',
    tables: ['CONSUMABLE_LOGS'],
    processPattern: [/입고등록/, /등록/, /입고/],
  },
  CONS_ISSUING: {
    slug: 'consumables-issuing',
    route: '/consumables/issuing',
    title: '출고관리',
    api: '/consumables/logs?limit=20',
    tables: ['CONSUMABLE_LOGS'],
    processPattern: [/출고등록/, /등록/, /출고/],
  },
  CONS_STOCK: {
    slug: 'consumables-stock',
    route: '/consumables/stock',
    title: '재고현황조회',
    api: '/consumables/stocks',
    tables: ['CONSUMABLE_STOCKS'],
    processPattern: [/조회/, /검색/],
  },
  CONS_LIFE: {
    slug: 'consumables-life',
    route: '/consumables/life',
    title: '수명현황조회',
    api: '/consumables/life-status?limit=5000',
    tables: ['CONSUMABLE_MASTERS'],
    processPattern: [/조회/, /검색/],
  },
  CONS_MOUNT: {
    slug: 'consumables-mount',
    route: '/consumables/mount',
    title: '장착관리',
    api: '/equipment/consumables?limit=20',
    tables: ['CONSUMABLE_MOUNT_LOGS', 'CONSUMABLE_MASTERS'],
    processPattern: [/장착/, /mount/],
  },
};
...
async function discoverConsumablesMenus() {
  const tree = await apiGet('/menu-categories/tree');
  const categories = Array.isArray(tree) ? tree : tree.data ?? [];
  const category = categories.find((item) => item.categoryCode === 'CONSUMABLES' && item.isActive !== 'N');
  if (!category) throw new Error('CONSUMABLES menu category not found in /menu-categories/tree');
  ...
  await addStep('save-validation-policy', '저장 검증 및 중복 방어 정책', `${pageInfo.title} 화면의 저장성 처리 경계와 중복 방어 검증 방식을 기록합니다.`, [
    ...
    return {
      ...
      notes: [
        ...
        `중복 방어 공통 시나리오: 동일 소모품코드/동일 conUid 재처리 방어는 페이지별 저장형 상세 시나리오에서 수행`,
        `저장성 후보 버튼: ${buttons.filter((name) => /등록|저장|추가|삭제|수정|취소|입고|출고|분할|병합|폐기|보정|특채|승인|반려/.test(name)).join(', ') || '없음'}`,
      ],
    };
  });
...
function renderPageReport(pageResult) {
  const featureRows = [
    ['좌측 메뉴 등록', `/menu-categories/tree CONSUMABLES/${pageResult.menuCode}`, '실행', `등록 순서 ${pageResult.sortOrder}; 실제 등록 메뉴 기준`],
    ...
    ['중복 방어', '동일 소모품코드/conUid 재처리 방어', '별도 시나리오 대상', '공통 스윕에서는 방어 정책과 버튼/API 경계를 기록'],
...
<head>
  <meta charset="utf-8">
  <title>소모품관리 등록 메뉴 상세 시나리오 QA 목차</title>
  <style>
    body { margin: 0; font-family: Arial, "Malgun Gothic", sans-serif; background: #f5f7fb; color: #111827; }
    header { background: #172033; color: #fff; padding: 28px 34px; }
    main { padding: 24px 34px 48px; }
...
<body>
  <header>
    <h1>소모품관리 등록 메뉴 상세 시나리오 QA 목차</h1>
  ...
  <main>
    <section class="card">
      <p>이 보고서는 <code>/api/v1/menu-categories/tree</code>에서 <code>CONSUMABLES</code> 카테고리에 등록된 하위 메뉴만 대상으로 생성했습니다. 정적 메뉴에는 있으나 실제 좌측 메뉴에 등록되지 않은 화면은 제외했습니다.</p>
    </section>
    <section class="card">
      <table>
...
async function writeAggregate(category, pages, results) {
  ...
  await fs.writeFile(resultPath, JSON.stringify({
    ...
    menuSource: {
      ...
      excludedStaticConsumablesCodes: Object.keys(staticMenuMap).filter((code) => !pages.some((page) => page.menuCode === code)),
    },
    pages: results,
  }, null, 2), 'utf8');
...
async function main() {
  ...
  const { category, pages } = await discoverConsumablesMenus();
  if (aggregateMode) {
    const results = [];
    ...
    console.log(JSON.stringify({ status: aggregate.status, source: '/menu-categories/tree CONSUMABLES', pages: results.length, passed: results.filter(r => r.status === 'PASS').length, failed: aggregate.failed.length, indexPath, resultPath }, null, 2));
  ...
  console.log(JSON.stringify({
    ...
    source: '/menu-categories/tree CONSUMABLES',
